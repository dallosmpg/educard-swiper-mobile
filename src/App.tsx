import './App.css'
import hunQuestions from './assets/questions/hun.json'; 
import engQuestions from './assets/questions/eng.json'; 
import { useState } from 'react';

function App() {
  const [language, setLanguage] = useState('hun');
  const [isDark, setIsDark] = useState(false);
  const [questionNumber, setQuestionNumber] = useState(0);
  const questions = language === 'hun' ? hunQuestions.questions : engQuestions.questions;

  const changeTheme = () => {
    setIsDark(isDark => !isDark);
    document.documentElement.style.setProperty('--color-scheme', `${isDark ? 'dark' : 'light'}`);
  };

  // function updateNotes() {
  //   return null;
  // }

  function decreaseQuestionNumber() {
    console.log(questionNumber);
    if (questionNumber === 0) {
      setQuestionNumber(questions.length - 1);
      return;
    }
    setQuestionNumber(currQuestionNum => currQuestionNum - 1);
  }

  function increaseQuestionNumber() {
    console.log(questionNumber);
    if (questionNumber === questions.length - 1) {
      setQuestionNumber(0);
      return;
    }
    setQuestionNumber(currQuestionNum => currQuestionNum + 1);
  }

  return (
    <>
    <header>
      <select className='minimal-select' onInput={(e) => {
        const value: string = e.currentTarget.value;
        setLanguage(value)
        }} name="language" id="language">
        <option value="eng">English</option>
        <option selected value="hun">Magyar</option>
      </select>      

      <label className="switch">
          <span className="sun"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><g fill="#ffd43b"><circle r="5" cy="12" cx="12"></circle><path d="m21 13h-1a1 1 0 0 1 0-2h1a1 1 0 0 1 0 2zm-17 0h-1a1 1 0 0 1 0-2h1a1 1 0 0 1 0 2zm13.66-5.66a1 1 0 0 1 -.66-.29 1 1 0 0 1 0-1.41l.71-.71a1 1 0 1 1 1.41 1.41l-.71.71a1 1 0 0 1 -.75.29zm-12.02 12.02a1 1 0 0 1 -.71-.29 1 1 0 0 1 0-1.41l.71-.66a1 1 0 0 1 1.41 1.41l-.71.71a1 1 0 0 1 -.7.24zm6.36-14.36a1 1 0 0 1 -1-1v-1a1 1 0 0 1 2 0v1a1 1 0 0 1 -1 1zm0 17a1 1 0 0 1 -1-1v-1a1 1 0 0 1 2 0v1a1 1 0 0 1 -1 1zm-5.66-14.66a1 1 0 0 1 -.7-.29l-.71-.71a1 1 0 0 1 1.41-1.41l.71.71a1 1 0 0 1 0 1.41 1 1 0 0 1 -.71.29zm12.02 12.02a1 1 0 0 1 -.7-.29l-.66-.71a1 1 0 0 1 1.36-1.36l.71.71a1 1 0 0 1 0 1.41 1 1 0 0 1 -.71.24z"></path></g></svg></span>
          <span className="moon"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path d="m223.5 32c-123.5 0-223.5 100.3-223.5 224s100 224 223.5 224c60.6 0 115.5-24.2 155.8-63.4 5-4.9 6.3-12.5 3.1-18.7s-10.1-9.7-17-8.5c-9.8 1.7-19.8 2.6-30.1 2.6-96.9 0-175.5-78.8-175.5-176 0-65.8 36-123.1 89.3-153.3 6.1-3.5 9.2-10.5 7.7-17.3s-7.3-11.9-14.3-12.5c-6.3-.5-12.6-.8-19-.8z"></path></svg></span>   
          <input onChange={changeTheme} checked={!isDark} type="checkbox" className="input" />
          <span className="slider"></span>
      </label>

    </header>
    <main>
      <div className='flex-center-column'>
        <h1>Educards</h1>
        <h3>From Trash to Treasure</h3>
      </div>
      <div className='card-wrapper flex-center'>
          <svg className='left' onClick={decreaseQuestionNumber} xmlns="http://www.w3.org/2000/svg" width="150" height="150" viewBox="0 0 24 24"><path fill="currentColor" d="m3.55 12l7.35 7.35q.375.375.363.875t-.388.875q-.375.375-.875.375t-.875-.375l-7.7-7.675q-.3-.3-.45-.675T.825 12q0-.375.15-.75t.45-.675l7.7-7.7q.375-.375.888-.363t.887.388q.375.375.375.875t-.375.875L3.55 12Z"/></svg>
          <p>
            {questions[questionNumber]}
          </p>
          <svg className='right' onClick={increaseQuestionNumber} style={{transform: 'rotate(180deg)'}} xmlns="http://www.w3.org/2000/svg" width="150" height="150" viewBox="0 0 24 24"><path fill="currentColor" d="m3.55 12l7.35 7.35q.375.375.363.875t-.388.875q-.375.375-.875.375t-.875-.375l-7.7-7.675q-.3-.3-.45-.675T.825 12q0-.375.15-.75t.45-.675l7.7-7.7q.375-.375.888-.363t.887.388q.375.375.375.875t-.375.875L3.55 12Z"/></svg>
      </div>  
      <textarea placeholder='Your notes...' name="notes" id="notes" cols={50} rows={20}></textarea>
    </main>
    <footer>

    </footer>
    </>
  )
}

export default App
